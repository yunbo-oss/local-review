...
用户请求 POST /api/voucher-order/seckill/:id
    │
    ├─ 1. 布隆过滤器校验 voucherId（不存在直接 404）
    ├─ 2. 校验秒杀时间、库存、用户是否已购买
    ├─ 3. 令牌桶限流（超限 429）
    ├─ 4. 发送 RocketMQ 事务消息（半消息）
    │       └─ ExecuteLocalTransaction：执行 Lua 预减 Redis + 防重复
    │       └─ 成功 → Commit；失败 → Rollback
    ├─ 5. 立即返回「排队中」
    │
    └─ 消费者异步：createVoucherOrder（事务：重复校验 + DecrStock + Create）
```
...
- **消费者**：`HandleOrderTimeout` → `UpdateStatus` 关单 → MySQL `IncrStock` 回滚库存 → Lua 回滚 Redis 库存与 `SADD` 标记
...
- **当前使用**：`QueryShopByIdWithCacheNull`，Cache Aside + 布隆过滤器防穿透
