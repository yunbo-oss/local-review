...
本文只记录本轮从“代码存在但从未真实运行”推进到可复现评测闭环时遇到的问题。正式实验条件、最终指标和演示命令见 `AGENT_AND_EVAL.md`；这里出现的中间分数均为诊断数据，不可用于简历或项目结论。
...
**剩余限制**：RocketMQ Broker 在本机 Docker Desktop 上内存占用较高，面试演示前应预留约 1 GB。
